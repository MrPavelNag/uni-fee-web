...
#   BYPASS_SYNC_CONFIRM=1 ./scripts/deploy_sync.sh "non-interactive run"
#
...
BYPASS_SYNC_CONFIRM="${BYPASS_SYNC_CONFIRM:-0}"

require_sync_confirmation() {
  if [[ "${REQUIRE_MANUAL_CONFIRM}" != "1" ]]; then
    return
  fi
  if [[ "${BYPASS_SYNC_CONFIRM}" == "1" ]]; then
    echo "==> Sync confirmation bypassed (BYPASS_SYNC_CONFIRM=1)"
    return
  fi
  if [[ -t 0 ]]; then
    local prompt
    prompt="Type 'YES' to commit+push sync to ${TARGET_BRANCH}: "
    read -r -p "${prompt}" answer
    if [[ "${answer}" != "YES" ]]; then
      echo "Aborted: sync confirmation failed."
      exit 1
    fi
    return
  fi
  echo "Error: non-interactive sync blocked by safety guard."
  echo "Use BYPASS_SYNC_CONFIRM=1 to allow non-interactive commit+push."
  exit 1
}
